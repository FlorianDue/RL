...
from Agents.utils.learning_and_exploration import double_q_learning, learning, q_learning
...
if __name__ == '__main__':
    current_network, target_network = set_up_networks()
    #strategy = double_q_learning
    agent = train_agent(action_space = [0, 1],
                        replace = 100,
    ...
    #print("q learning")
    #agent.set_networks(current_network, target_network)
    #agent.set_learning_strategy(learning, q_learning)
    #agent.train(training_steps = 500, environment = env, print_results = True, average = 50)
    
    print("double q learning")
    current_network, target_network = set_up_networks()
    agent.memory.reset_memory(input_dims = 14, batch_size = 512)
    agent.reset_epsilon(1)
    agent.set_networks(current_network, target_network)
